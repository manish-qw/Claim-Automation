        ...
        if isinstance(docs, list):
            ...
            for d in docs:
                if isinstance(d, dict):
                    ...
                    source_doc = metadata.get("raw_document") if isinstance(metadata.get("raw_document"), dict) else None
                    if source_doc is None and isinstance(d.get("extracted_entities"), dict):
                        source_doc = d.get("extracted_entities")
                    if source_doc is None:
                        source_doc = {k: v for k, v in d.items() if k != "metadata"}
